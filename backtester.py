    ...
    def backtest(self):
        ...
        while self.day <= self.end_date:
            ...
            if self.status == 'buy':
                ...
                for s in stocks:
                    recommended_action, current_price = LR_v1_sell(s, self.buy_orders[s][3], self.buy_orders[s][0],
                                                                   self.day, self.sell_perc, self.hold_till,
                                                                   self.stop_perc)
...
if __name__ == "__main__":
    ...
    back = backtester(stock_list, LR_v1_predict, 100000, datetime(2024, 1, 1), datetime(2024, 12, 13), threshold=0.9,
                      sell_perc=0.03, hold_till=10, \
                      stop_perc=0.03)
    back.backtest()

# todo change trade date