...
class simulator:
    ...
    def create_summary(self, print_results=False):
        ...
        if print_results:
            print(
                "{:<10} {:<10} {:<10} {:<10} {:<10} {:<10} {:<10}".format('STOCK', 'BUY PRICE', 'SHARES', 'SELL PRICE',
                                                                          'NET GAIN', 'BUY DATE', 'SELL DATE'))
        ...
        for values in self.history:
            ...
            if print_results:
                print("{:<10} {:<10} {:<10} {:<10} {:<10} {:<10} {:<10}".format(values[0], values[1], values[2],
                                                                                values[3],
                                                                                pd.to_datetime(values[4]).strftime(
                                                                                    '%Y-%m-%d'),  # 格式化 buy_date
                                                                                pd.to_datetime(values[5]).strftime(
                                                                                    '%Y-%m-%d'),  # 格式化 sell_date
                                                                                np.round(net_gain, 2)))
